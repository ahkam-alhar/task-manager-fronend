    ...
    useEffect(() => {
        if (allTasks) {
            ...
            allTasks.map((task) => {
                ...
                if (priority === 1) {
                    if (status === 1)
                        highPrioPendingTaskList.push(task);
                    else if (status === 0)
                        highPrioOpenTaskList.push(task);

                } else if (priority === 0) {
                    if (status === 1)
                        mediumPrioPendingTaskList.push(task);
                    else if (status === 0) 
                        mediumPrioOpenTaskList.push(task);

                } else if (priority === -1) {
                    if (status === 1)
                        lowPrioPendingTaskList.push(task);
                    else if (status === 0)
                        lowPrioOpenTaskList.push(task);
            ...
            setLowMedPrioPendingTasks([...mediumPrioPendingTaskList, ...lowPrioPendingTaskList]);
            setOpenTasks([...highPrioOpenTaskList, ...mediumPrioOpenTaskList, ...lowPrioOpenTaskList]);
        }
    }, [allTasks]);
