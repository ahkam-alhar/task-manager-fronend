            ...
            <div className={`card ${classOverride} mb-3 h-100`}>
                ...
                <div className="card-body overflow-auto">
                    <ul className="list-group list-group-flush">
                        {dataSet && dataSet.map((value, key) => {
                            return (<li className="list-group-item d-flex justify-content-between" key={key}>
                                    <p className="mr-auto">{value.task}</p>
                                    <div className='d-flex'>
                                        <div className={`${value.cssText} px-2`}>{value.priorityValue}</div>
                                        {additionalTag}
                                    </div>
                            </li>)
                        })}
